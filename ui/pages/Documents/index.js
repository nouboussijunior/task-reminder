...
import { StyledDocuments, DocumentsList, Document, StyledListGroup, StyledListGroupItem } from './styles';
import { documents } from '../../queries/Documents.gql';
import { addDocument as addDocumentMutation, 
        ...
        <Row>
          <Col md={6}>
            ...
            {data.documents && data.documents.length ? (
              <StyledListGroup>
                {data.documents.map(({ _id, isPublic, taskName, deadLine, updatedAt, status }) => (
                  status == "pending" &&
                    <StyledListGroupItem key={_id}>
                    {/* <Link to={`/documents/${_id}/edit`} /> */}
                    <div className="taskItem">
                      <Col xs={1}>
                        <input type="checkbox" className="resolveTask" onChange={()=>this.resolveTask(_id)}/>
                      </Col>  
                      <Col xs={10}>
                        ...
                        <h3 >{taskName}</h3>  
                        {/* <p>{deadLine}</p> */}
                        <span className={"label label-"+ this.getTaskPeriod(deadLine)}>{timeago(deadLine)}</span>
